...
public class hModelTest {
    ...
    @BeforeEach
    public void setup() {
        model = new hModel(new Model());
        model.setGuestModel(serverAddress, serverPort);
    }

    @Test
    public void testSendRequestToServer() throws IOException {
        ...
        String response = model.sendRequestToServer(query);

        // Restore System.out
        System.setOut(originalOut);

        // Verify the response
        Assertions.assertEquals("Response from server", response, "Response should match the expected value");
